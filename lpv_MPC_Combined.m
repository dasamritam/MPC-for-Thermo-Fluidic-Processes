...
% Choosing State and input Weights
Q = diag ([10 10 10 10 10 10 10 10 repmat([1000 10],1,N_nz)] );
...
opt = optimoptions('lsqlin','Algorithm','interior-point','MaxIterations',800,'display','off');
aaa=0;
...
SHinv((N_per+1)*nx+1:(N_per+1)*nx+N_nz,1:nx) = C_r;
for i = 1:N_per
...
%% Receeding horizon optimisation
    ...
    options = optimoptions(@quadprog,'Display','off');
    tic
        [u_qp1,fval,exitflag] = quadprog(G ,F*xh(:,k)+Y+H ,L,W*xh(:,k)+c-Mcal*Affine_Eta_del,[],[],[],[],[],options);
    t_quad(k)=toc;
%     tic
%     if k==1
